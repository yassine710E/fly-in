...
class Parser:
    ...
    def check_hubs(self, line: str) -> dict:
        ...
        match_2 = re.match(check_str, striped_values)
        if match_2 is None:
        ...
        dict_metadata: dict[str, Any] = dict(re.findall(
            r"(color|zone|max_drones)\s*=\s*([^\s=\]]+)", striped_values))
        for k, v in dict_metadata.items():
            if k == "zone":
                list_zone = self.metadata[k]
                if (isinstance(list_zone, list) and v not in list_zone):
                ...
                list_colors = self.metadata[k]
                if isinstance(list_colors, list) and v not in list_colors:
    ...
    def check_connection(self, line: str) -> dict:
        ...
        match_1 = tuple(striped_values.split()[0].split('-'))
        if match_1[0] == match_1[1]:
            return {'error': 'duplicated names of hub '
                    f'"\033[4m{match_1[0]}\033[0m" in one connection'}
        prev_tuple_connections = [connection['tuple_connections']
                                  for connection
                                  in self.config_data['connections']]
        if match_1 in prev_tuple_connections:
            return {'error': f'"\033[4m{match_1}\033[0m" '
                    'already exist in previous connections'}
        if match_1[::-1] in prev_tuple_connections:
            return {'error': f'reversed "\033[4m{match_1}\033[0m" '
                    'already exist in previous connections'}

        # set tuple connection
        clear_value['tuple_connections'] = match_1
        ...
        connections_and_metadata: list = striped_values.split()
