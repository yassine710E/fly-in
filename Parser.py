...
def raise_for_order_prop(order,index_checker,prop,index)->None:
        if order > index_checker:
            raise ParsingError(f'prop "\033[4m{prop}\033[0m" is not ordered in line {index+1}')
...
class Parser:
    def __init__(self, file_name: str) -> None:
        ...
        self.prop_flags = {'nb_drones':False,'start_hub':False,'end_hub':False,'hub':False,'connection':False}
    ...
    def check_hubs(self,line:str) -> dict:
        ...
        correct_value = dict()
        if len(splited_line) != 2:
            return {'error':'line doesn\'t respect this pattern "\033[4mkey : value\033[0m"' }
        striped_prop:str = (splited_line[0]).strip()
        striped_values : str = (splited_line[1]).strip()
        
        # 1st : check the name of hub 
        check_string_pattern = r"^[^-\s]+\s+"
        match = re.findall(check_string_pattern,striped_values)
        if not match :
            return {'error': f'name of hub is not valid "\033[4m{striped_values.split()[0]}\033[0m"'}
        
        if match[0].strip() in [zone['name']for zone in self.config_data['zones']]:
            return {'error':f'name of hub "\033[4m{match[0].strip()}\033[0m" is already exist'}

        

        correct_value['name'] = match[0].strip()
        # 2nd : check coordinates of hub
        check_string_pattern += r"-?\d+\s+-?\d+"
        match = re.findall(check_string_pattern,striped_values)
        if not match:
            err_data = striped_values.split()[1] if not is_int(striped_values.split()[1]) else striped_values.split()[2]
            return {'error':f'coordinates of hub  are not valid "\033[4m{err_data}\033[0m"'}
        correct_value['coordinates']= (int(match[0].split()[1]),int(match[0].split()[2]))
        
        # 3rd : check metadata
        check_string_pattern += r"(\s+\[\s*(color|zone|max_drones)\s*=\s*[^\s=\]]+(\s+(color|zone|max_drones)\s*=\s*[^\s=\]]+)*\s*\])?$"
        match = re.match(check_string_pattern,striped_values)
        if match is None:
            return {'error' : f'this part is not respecting the metadata pattern   "\033[4m[color:... zone:... max_drones:...]\033[0m"'}
        arr_props : list[str] = re.findall(r"[^:]\s*(color|zone|max_drones)",striped_values)
        if get_duplicated_prop(arr_props) is not None:
            return {'error': f'duplicted prop "\033[4m{get_duplicated_prop(arr_props)}\033[0m"'}
        ...
        for k,v in dict_metadata.items():
            if k == "zone" and v not in self.metadata[k]:
                return {'error' : f'zone value in metadata is not validated "\033[4m{v}\033[0m"'}
            elif k == "max_drones":
                try :
                    dict_metadata[k] = int(v)
                    if dict_metadata[k] < 1:
                        return {'error' : f'not valid integer less than 1 "\033[4m{v}\033[0m"'}
                except ValueError as e:
                    return  {'error':f'max_drones is not valid integer "\033[4m{v}\033[0m"'}
        ...
        if striped_prop in ['end_hub','start_hub'] and striped_prop in [zone['type'] for zone in self.config_data['zones']]:
            return {'error': f'"\033[4m{striped_prop}\033[0m" is duplicated and already exist'}
        if correct_value['coordinates'] in [zone['coordinates'] for zone in self.config_data['zones']]:
            return {'error':f'coordinates are already exist'}
    ...
    def check_connection(self,line:str)->None|tuple:
        ...
        if  match is None:
            return {'error':f'there a problem in connection "\033[4m{striped_values.split()[0]}\033[0m" it should be line "\033[4mzone-zone (and zone must be exist in hubs config data)\033[0m"'}
        match = tuple(striped_values.split()[0].split('-'))
        if match[0] == match[1]:
            return {'error' : f'duplicated names of hub "\033[4m{match[0]}\033[0m" in one connection'}
        prev_tuple_connections = [connection['tuple_connections'] for connection in self.config_data['connections'] if 'connections']
        if match in prev_tuple_connections :
            return {'error':f'"\033[4m{match}\033[0m" already exist in previous connections'}
        if  match[::-1] in prev_tuple_connections:
            return {'error':f'reversed "\033[4m{match}\033[0m" already exist in previous connections'}
        
        # set tuple connection 
        clear_value['tuple_connections'] = match

        # 2nd : check the metadata if valid or not
        checker_pattern += fr"(\s+\[\s*max_link_capacity\s*=\s*[1-9]\d*\s*\])?$"
        match = re.search(checker_pattern,striped_values)
        if match is None:
            return {'error':f'there is a problem in metadata "\033[4m{striped_values.split()[1]}\033[0m'} 
    ...
    def parsing(self)->dict:
        ...
        lines_without_comments : list[str] = [line for line in self.content if line and not re.search(r"^#",line) ]
        ...
        checkers_list = [self.check_nb_drones,self.check_hubs,self.check_connection]
        index_checker = 0
        order = 0
        flag_hubs = False
        ...
        for index,line in enumerate(self.content):
            ...
            if 'error' in checker_returned_value:
                raise ParsingError(f"Error : {checker_returned_value['error']} in line {index + 1}")
        ...
        for prop,flag in self.prop_flags.items():
            if not flag and prop != "hub":
                raise ParsingError(f'Error : key "\033[4m{prop}\033[0m" doesn\'t exist')
